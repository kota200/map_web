...
- **Phase W6 in progress:** Chrome 151 passed Kallisto fastp OFF/ON, cancellation/cleanup, resource/checksum/annotation-mismatch failures, the existing Kallisto baseline, and the W5 HISAT2 fixture gate. A fresh instrumented run built the supplied Arabidopsis index in 201 seconds with a 1,120,010,240-byte Wasm linear-memory high water, then quantified the supplied 3.10 GB paired gzip FASTQ in 882 seconds with a 747.8 MiB rounded high-water observation. This measures Wasm allocation, not OS resident memory. Production HISAT2 biology/hosting, storage eviction at production scale, Firefox, and Safari remain open.
- **Desktop remains unimplemented:** the Web acceptance gate must pass first.
