...
|---|---|---|---|---|
| kallisto | 0.52.0, packaged and tested | D2 native 0.52.0 runner implemented; platform artifacts pending clean CI | `vendor/kallisto` tag `v0.52.0`, commit `4e9f29cf3b021260415430c057a22469ca081391`; clean native source is fetched at that revision | Web: Memory64/pthread/O3; desktop: HDF5/BAM off, plaintext bootstrap output, portable platform flags, exact binary SHA in generated manifest |
...
| Tauri | Not applicable | Rust crate 2.11.5 locked; CLI 2.11.4 pinned for D2 unsigned installer CI | `desktop/src-tauri/Cargo.lock`, platform workflow definitions | Tauri 2 external binaries and target-specific manifests; D1 Windows build passed, D2 installers pending CI and signing |
| Rust | Not applicable | GitHub-hosted target toolchains only for current desktop verification | Merged D1 run `32437427175`; D2 workflows record runner/target evidence | Local Rust execution remains avoided because endpoint policy blocks the unsigned local toolchain |
...
The W2 proof artifact hashes and complete flags are locked in each tool's `artifacts.lock.json`. D1 Windows acceptance is recorded on merged `main`. D2 workflow definitions calculate target-specific sidecar and installer hashes, but those rows remain pending until their clean CI runs pass; unsigned installers are not release-ready.
