import { runHisat2 } from './hisat2-runner.mjs';
...
async function createHost() {
  ...
  if (!parentPort) throw new Error('HISAT2 worker requires a Worker host.');
  return {
...
host.onMessage(async (message) => {
  ...
  if (active) {
    host.post({ type: 'error', message: 'A HISAT2 run is already active.', exitCode: null });
    return;
  }
  active = true;
  host.post({ type: 'started' });
  try {
    const result = await runHisat2(message.config, {
