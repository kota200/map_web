...
async function createFeatureCountsModule(moduleArg = {}) {
...
function findWasmBinary() {
  if (Module["locateFile"]) {
    return locateFile("featureCounts.wasm");
  }
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  return new URL("featureCounts.wasm", import.meta.url).href;
}
...
var PThread = {
  ...
  allocateUnusedWorker() {
    ...
    worker = new Worker(new URL("featureCounts.mjs", import.meta.url), {
      "type": "module",
...
function ___syscall_getdents64(fd, dirp, count) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(4, 0, 1, fd, dirp, count);
  try {
    var stream = SYSCALLS.getStreamFromFD(fd);
...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(7, 0, 1, olddirfd, oldpath, newdirfd, newpath);
  try {
...
function ___syscall_socket(domain, type, protocol, u1, u2, u3) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(8, 0, 1, domain, type, protocol, u1, u2, u3);
  try {
...
function ___syscall_unlinkat(dirfd, path, flags) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 0, 1, dirfd, path, flags);
  try {
    path = SYSCALLS.getStr(path);
    path = SYSCALLS.calculateAt(dirfd, path);
    if (!flags) {
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_fcntl64, ___syscall_getdents64, ___syscall_ioctl, ___syscall_openat, ___syscall_renameat, ___syscall_socket, ___syscall_unlinkat, _fd_close, _fd_read, _fd_seek, _fd_write ];
...
export default createFeatureCountsModule;
...
isPthread && createFeatureCountsModule();
