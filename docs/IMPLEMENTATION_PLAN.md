...
| Phase W6 | In progress; not release-complete | Chrome completed the instrumented 3.10 GB Kallisto run. Chromium 140 and Firefox 141 passed the automated Kallisto/W5/W6 small gates in run `32458539850`; Firefox is Experimental. WebKit 26 failed required runtime/OPFS diagnostics and does not substitute for Safari. Production HISAT2 hosting/scale, eviction/recovery at production scale, and real Safari remain open. |
...
7. **W6 production Web release gate — in progress**
   ...
   - Run `32458539850` passed the archived Kallisto regression, W5 synthetic HISAT2 pipeline, and W6 failures/cleanup gate in Chromium 140 and Firefox 141. Firefox is Experimental until representative-scale evidence exists. Playwright WebKit 26 lacked OPFS and the packaged Kallisto runtime; real Safari remains unmeasured and Unsupported.
   - Keep the release gate open because the production HISAT2 catalog/CDN is unapproved, representative HISAT2 Web resource/scientific agreement and eviction/recovery are unmeasured, and real Safari has not been tested. Wasm allocation is measured for the representative Kallisto run, but OS/browser-process resident memory and a portable support threshold are not claimed.
